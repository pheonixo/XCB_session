...
static void
_findport_keyboard(PhxObjectTextview *otxt, uint8_t key) {
...
static bool
_textview_meter(PhxInterface *iface,
                xcb_generic_event_t *nvt, PhxObject *obj) {

  if ( ((nvt->response_type & (uint8_t)0x7F) == XCB_KEY_PRESS)
      || ((nvt->response_type & (uint8_t)0x7F) == XCB_KEY_RELEASE)  ) {

    xcb_key_press_event_t *kp;
    xcb_keysym_t keyval;
    uint16_t state;
    bool locus = ((nvt->response_type & (uint8_t)0x7F) == XCB_KEY_RELEASE);
    ...
    if ((state & XCB_MOD_MASK_CONTROL) != 0) {
      if (keyval == 'e') {
        if (locus) {
          puts("⌃e");
          _findport_keyboard(obj, (uint8_t)keyval);
        }
        return true;
      }
      if (keyval == 'E') {
...
user_textstring(void) {
  ...
  if (fread(string, filesize, 1, rh) == 0)
    puts("error: reading file xcb_fbar.c");
  fclose(rh);
...
user_configure_layout(PhxInterface *iface) {
  ...
  ui_window_minimum_set(window, BOX_MIN_WIDTH, BOX_HEIGHT + 1);

  RECTANGLE(nexus_box, 0, 0, iface->mete_box.w, iface->mete_box.h);
  nexus = ui_nexus_create(iface, nexus_box);
  nexus->state |= HXPD_LFT | VXPD_BTM;

  otxt = ui_textview_create(nexus, nexus_box);
  otxt->_event_cb = _textview_meter;
  otxt->attrib->fg_fill = RGBA_TEXT_FGFILL;
...
main(int argc, char *argv[]) {
  ...
  PhxRectangle configure = { 1000, 300, 600, 200 + (BOX_HEIGHT * 2) };
