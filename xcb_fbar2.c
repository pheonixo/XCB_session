...
void
_findport_search(PhxInterface *iface, PhxObjectTextview *otxt) {
...
_findport_create(PhxInterface *iface, PhxRectangle configure) {
  ...
  ui_visible_set((PhxObject*)nexus, true);
  return nexus;
}
...
ui_findport_create(PhxInterface *iface, PhxObjectTextview *otxt) {
  ...
  configure = otxt->i_mount->mete_box;
    ...
    /* Dummy code, see xcb_fbar.c */
  configure.h = BOX_HEIGHT * 2;
    /* row 0 larger due to draw_box offset */
  ui_window_minimum_set(iface->window, BOX_MIN_WIDTH, BOX_HEIGHT + 1);
...
user_configure_layout(PhxInterface *iface) {
  ...
  PhxRectangle findport_box;

    /* Currently only names as window id */
  xcb_window_t window = iface->window;
  ui_window_name(window);

    /* Artifical heights for viewing of findport creation design. */
  RECTANGLE(findport_box, 0, 0, iface->mete_box.w, BOX_HEIGHT * 2);
  if (BOX_HEIGHT < 21) findport_box.h++;
  _findport_create(iface, findport_box);

  findport_box.y += 2;

  findport_box.y += BOX_HEIGHT * 2;
  findport_box.h = (BOX_HEIGHT - 1) * 2;
  if ((BOX_HEIGHT - 1) < 21) findport_box.h++;
  _findport_create(iface, findport_box);

  findport_box.y += 2;

  findport_box.y += (BOX_HEIGHT - 1) * 2;
  findport_box.h = (BOX_HEIGHT - 2) * 2;
  if ((BOX_HEIGHT - 2) < 21) findport_box.h++;
  _findport_create(iface, findport_box);

  findport_box.y += 2;

  findport_box.y += (BOX_HEIGHT - 2) * 2;
  findport_box.h = (BOX_HEIGHT - 3) * 2;
  if ((BOX_HEIGHT - 3) < 21) findport_box.h++;
  _findport_create(iface, findport_box);

  findport_box.y += 2;

  findport_box.y += (BOX_HEIGHT - 3) * 2;
  findport_box.h = (BOX_HEIGHT - 4) * 2;
  if ((BOX_HEIGHT - 4) < 21) findport_box.h++;
  _findport_create(iface, findport_box);

  findport_box.y += 2;

  findport_box.y += (BOX_HEIGHT - 4) * 2;
  findport_box.h = (BOX_HEIGHT - 5) * 2;
  if ((BOX_HEIGHT - 5) < 21) findport_box.h++;
  _findport_create(iface, findport_box);

  findport_box.y += 2;

  findport_box.y += (BOX_HEIGHT - 5) * 2;
  findport_box.h = (BOX_HEIGHT - 6) * 2;
  if ((BOX_HEIGHT - 6) < 21) findport_box.h++;
  _findport_create(iface, findport_box);

  findport_box.y += 2;

  findport_box.y += (BOX_HEIGHT - 6) * 2;
  findport_box.h = (BOX_HEIGHT - 7) * 2;
  if ((BOX_HEIGHT - 7) < 21) findport_box.h++;
  _findport_create(iface, findport_box);
...
main(int argc, char *argv[]) {
  ...
  PhxRectangle configure = { 1000, 300, 600, 320 };
    /* need code for window below BOX_MIN_WIDTH, should
      run as dialog rather than finbar. */
  configure.w = maxof(600, BOX_MIN_WIDTH);
